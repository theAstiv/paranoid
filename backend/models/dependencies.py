...
class CapabilityProfile(BaseModel):
    ...
    # GitHub only: names of `backend.deps.fetcher._EXTERNAL_BUILD_MARKERS`
    # found at the repo root (Makefile, go.mod, Cargo.toml, ...) — a signal
    # that this package's shipped files may be generated by a non-npm build
    # toolchain rather than committed to the repo. Never excuses anything by
    # itself; see `backend.deps.drift._is_generated_unverifiable`.
    external_build_markers: list[str] = Field(default_factory=list)
...
class DriftReport(BaseModel):
    ...
    *categories* a file is excused for — it never excuses all of them by
    ...
    # Tarball-only files that are declared by the *GitHub* repo's own
    # manifest (main/bin/exports, or a lifecycle-hook's own `node <file>`
    # target — never the tarball's own manifest, which is
    # attacker-controlled) AND the GitHub repo has a non-npm build-toolchain
    # marker at its root (Makefile, go.mod, Cargo.toml, ...) — e.g. esbuild's
    # bin/esbuild, install.js and lib/main.js, generated by an external
    # Makefile and never committed. Always excluded from `signal`/
    # `signal_files`: without a repo-wide scan there's no reliable excuse set
    # to compare a generated file's categories against, so this bucket is
    # capped at informational rather than risking a false "no signal" on a
    # genuinely novel capability. See `backend.deps.drift._is_generated_unverifiable`.
    generated_unverifiable: list[str] = Field(default_factory=list)
    # {file: [categories absent from the whole GitHub scan]} for every
    # `generated_unverifiable` file — categories present *somewhere else* in
    # the GitHub scan go to `relocated` instead (that's exactly what
    # "relocated" means), but a category absent everywhere can't be called
    # relocated (there's nothing to relocate to). Neither ever contributes to
    # `signal`/`signal_files`, unlike `_record`'s normal strong/weak split —
    # this whole bucket is informational only.
    generated_unverifiable_categories: dict[str, list[CapabilityCategory]] = Field(
