...
async def test_list_threat_models(test_db):
    ...
    await crud.create_threat_model("Model 1", "Desc 1", "anthropic", "claude-sonnet-4")
    await crud.create_threat_model("Model 2", "Desc 2", "openai", "gpt-4")
    await crud.create_threat_model("Model 3", "Desc 3", "ollama", "llama3")

    # List models
    models = await crud.list_threat_models()
    assert len(models) == 3
...
async def test_create_and_get_threat(test_db):
    ...
    model_id = await crud.create_threat_model("Test Model", "Desc", "anthropic", "claude-sonnet-4")
...
async def test_list_threats_with_filter(test_db):
    ...
    threat1_id = await crud.create_threat(
        model_id, "Threat 1", "Desc 1", "Target", "High", "Low", ["Mit"]
    )
    threat2_id = await crud.create_threat(
        model_id, "Threat 2", "Desc 2", "Target", "High", "Low", ["Mit"]
    )
    threat3_id = await crud.create_threat(
        model_id, "Threat 3", "Desc 3", "Target", "High", "Low", ["Mit"]
    )

    # Update statuses
    await crud.update_threat_status(threat1_id, "approved")
    await crud.update_threat_status(threat3_id, "approved")

    # List all threats
    all_threats = await crud.list_threats(model_id)
    assert len(all_threats) == 3

    # List only approved threats
    approved_threats = await crud.list_threats(model_id, status="approved")
    assert len(approved_threats) == 2

    # List only pending threats
    pending_threats = await crud.list_threats(model_id, status="pending")
...
async def test_delete_threat(test_db):
    ...
    threat_id = await crud.create_threat(
        model_id, "Threat", "Desc", "Target", "High", "Low", ["Mit"]
    )
...
async def test_create_and_list_assets(test_db):
    ...
    # Create assets
    await crud.create_asset(model_id, "Asset", "Database", "PostgreSQL database")
    await crud.create_asset(model_id, "Entity", "User", "End user")
...
async def test_update_asset(test_db):
    ...
    asset_id = await crud.create_asset(model_id, "Asset", "API", "REST API")
...
async def test_create_and_list_flows(test_db):
    ...
    await crud.create_flow(model_id, "data_flow", "Database query", "API", "Database")
...
async def test_cascade_delete_threat_model(test_db):
    ...
    await crud.create_asset(model_id, "Asset", "Database", "DB")
    await crud.create_flow(model_id, "flow", "Data", "A", "B")
